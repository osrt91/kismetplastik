...
const placeholderKeys: Record<TabKey, PlaceholderCard[]> = {
  ...
  urunler: [
    ...
    { titleKey: "tubePackaging", descKey: "tubePackagingDesc", icon: Package, accent: "from-amber-500/10 to-amber-600/5" },
...
export default function GaleriPage() {
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") setLightboxIndex((prev) => prev !== null ? Math.min(prev + 1, tabImages.length - 1) : null);
  ...
  const getTitle = (img: GalleryImage) => locale === "en" ? (img.title_en || img.title_tr) : img.title_tr;
  const getDesc = (img: GalleryImage) => locale === "en" ? (img.description_en || img.description_tr) : img.description_tr;
    ...
    <section className="bg-white dark:bg-neutral-900">
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 lg:px-6 lg:py-20">
        ...
        <AnimateOnScroll animation="fade-up">
          <div className="mb-12 flex flex-wrap justify-center gap-2">
            {tabDefs.map((tab) => {
              ...
              const count = images.filter((img) => img.category === tab.key).length;
              return (
                <button
                  key={tab.key}
                ...
                >
                  ...
                  {count > 0 && (
                    <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${
                      isActive ? "bg-white/20 text-white" : "bg-neutral-200 dark:bg-neutral-700 text-neutral-500 dark:text-neutral-400"
                    }`}>
                      {count}
                    </span>
                  )}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="mt-3 text-sm text-neutral-500">{g.loadingGallery}</p>
          </div>
        ) : hasImages ? (
          /* Real Images Grid */
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {tabImages.map((img, i) => (
              <AnimateOnScroll key={img.id} animation="fade-up" delay={i * 50}>
                <button
                  onClick={() => setLightboxIndex(i)}
                  className="group relative flex h-full w-full flex-col overflow-hidden rounded-2xl border border-neutral-100 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-left transition-all duration-300 hover:-translate-y-1 hover:border-primary-100 hover:shadow-xl"
                >
                  <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    <div className="absolute bottom-3 right-3 opacity-0 transition-all duration-300 group-hover:opacity-100">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white/90 shadow-lg backdrop-blur-sm">
                        <Camera size={14} className="text-primary-700" />
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-1 flex-col p-4">
                    <h3 className="mb-1 font-bold text-primary-900 dark:text-white line-clamp-1">
                      {getTitle(img)}
                    </h3>
                    {getDesc(img) && (
          ...
          /* Placeholder Cards (no images yet) */
          <>
      ...
      {lightboxIndex !== null && tabImages[lightboxIndex] && (
        ...
        >
          ...
          {lightboxIndex < tabImages.length - 1 && (
            <button
              onClick={(e) => { e.stopPropagation(); setLightboxIndex(lightboxIndex + 1); }}
          ...
          >
            <img
              src={tabImages[lightboxIndex].image_url}
              alt={getTitle(tabImages[lightboxIndex])}
            ...
            <div className="absolute bottom-0 left-0 right-0 rounded-b-lg bg-gradient-to-t from-black/70 to-transparent p-4 pt-8">
              <h3 className="text-lg font-bold text-white">
                {getTitle(tabImages[lightboxIndex])}
              </h3>
              {getDesc(tabImages[lightboxIndex]) && (
                <p className="mt-1 text-sm text-white/70">
                  {getDesc(tabImages[lightboxIndex])}
                </p>
              )}
              <p className="mt-2 text-xs text-white/40">
                {lightboxIndex + 1} / {tabImages.length}
              </p>
            </div>
          </div>
        </div>
