  ...
  return (
    <group ref={groupRef} position={[0, -0.5, 0]}>
      ...
      <mesh position={[0, 2.0, 0]}>
        ...
        <meshPhysicalMaterial color={bodyColor} transparent={isTransparent} opacity={isTransparent ? 0.55 : 0.95} roughness={roughness} metalness={0.05} clearcoat={isTransparent ? 1 : 0.3} clearcoatRoughness={0.1} ior={1.5} thickness={isTransparent ? 0.5 : 0} envMapIntensity={1.5} />
      </mesh>
      <mesh position={[0, 0.8, 0]}>
        <cylinderGeometry args={[0.55, 0.55, 2.0, 48]} />
        <meshPhysicalMaterial color={bodyColor} transparent={isTransparent} opacity={isTransparent ? 0.55 : 0.95} roughness={roughness} metalness={0.05} clearcoat={isTransparent ? 1 : 0.3} clearcoatRoughness={0.1} ior={1.5} thickness={isTransparent ? 1 : 0} envMapIntensity={1.5} />
      </mesh>
      <mesh position={[0, -0.4, 0]}>
        <cylinderGeometry args={[0.55, 0.50, 0.5, 48]} />
        <meshPhysicalMaterial color={bodyColor} transparent={isTransparent} opacity={isTransparent ? 0.55 : 0.95} roughness={roughness} metalness={0.05} clearcoat={isTransparent ? 1 : 0.3} clearcoatRoughness={0.1} />
      </mesh>
...
function Scene({ product, selectedColor }: Props) {
  ...
  return (
    <>
      ...
      <ContactShadows position={[0, -1, 0]} opacity={0.35} scale={4} blur={2} far={3} />
      <Environment preset="city" />
      <OrbitControls enablePan={false} minPolarAngle={Math.PI / 5} maxPolarAngle={Math.PI / 1.6} minDistance={3.5} maxDistance={7} autoRotate autoRotateSpeed={0.8} enableDamping dampingFactor={0.05} />
    </>
  );
}
...
export default function Product3DViewer({ product, selectedColor }: Props) {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative overflow-hidden rounded-2xl border border-neutral-200/60 bg-gradient-to-br from-neutral-50 via-primary-50/30 to-primary-100/20 dark:border-neutral-700 dark:from-neutral-800 dark:via-neutral-900 dark:to-neutral-800 ${
        isFullscreen ? "fixed inset-0 z-50 rounded-none" : ""
      }`}
      style={{ minHeight: isFullscreen ? "100vh" : 420 }}
    >
      {/* Controls — positioned below sticky header */}
      <div className="absolute right-3 top-3 z-10 flex gap-2" style={{ isolation: "isolate" }}>
        <button
          onClick={toggleFullscreen}
          className="rounded-xl bg-white/80 p-2 text-foreground/70 shadow-sm backdrop-blur-sm transition-colors hover:bg-white hover:text-primary-600 dark:bg-neutral-800/80 dark:text-neutral-300 dark:hover:bg-neutral-700 dark:hover:text-white"
      ...
      <div className="absolute bottom-3 left-1/2 z-10 -translate-x-1/2">
        <span className="rounded-full bg-white/70 px-3 py-1 text-[10px] font-medium text-muted-foreground backdrop-blur-sm dark:bg-neutral-800/70 dark:text-neutral-400">
