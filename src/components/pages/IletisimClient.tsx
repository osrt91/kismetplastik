...
export default function IletisimClient({ content, settings }: IletisimClientProps) {
  ...
  const emailValue = settings?.company_email || "bilgi@kismetplastik.com";
  const phoneHref = `tel:${(settings?.company_phone || "+902125498703").replace(/\s/g, "")}`;
  const emailHref = `mailto:${settings?.company_email || "bilgi@kismetplastik.com"}`;

  const contactItems = [
    { icon: Phone, label: c.phoneLabel, value: phoneValue, href: phoneHref },
    { icon: Mail, label: c.emailLabel, value: emailValue, href: emailHref },
    { icon: MapPin, label: c.addressLabel, value: settings?.company_address || c.addressValue, href: "https://maps.google.com" },
    { icon: Clock, label: c.hoursLabel, value: settings?.working_hours || c.hoursValue },
  ];
        ...
        <div className="relative mx-auto max-w-7xl px-4 lg:px-6">
          <AnimateOnScroll animation="fade-up">
            ...
            <h1 className="mb-4 text-3xl font-extrabold tracking-tight text-white sm:text-4xl lg:text-5xl">
              {t("contact_hero", "title", c.heroTitle)}
            </h1>
            <p className="max-w-2xl text-lg text-white/60">
              {t("contact_hero", "subtitle", c.heroSubtitle)}
            </p>
          </AnimateOnScroll>
        </div>
      </div>
