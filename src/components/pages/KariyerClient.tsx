...
import type { DbContentSection, DbCareerListing } from "@/types/database";

interface KariyerClientProps {
  content?: Record<string, DbContentSection>;
  settings?: Record<string, string>;
  listings?: DbCareerListing[];
}
...
export default function KariyerClient({ content, settings, listings }: KariyerClientProps) {
  ...
  /* Convert DB career listings to positions format, falling back to defaults */
  const positions: JobPosition[] =
    listings && listings.length > 0
      ? listings.map((l) => ({
          id: l.id,
          title: locale === "en" ? l.title_en : l.title_tr,
          department: l.department,
          location: l.location,
          type: l.type,
          description: locale === "en" ? l.description_en : l.description_tr,
          requirements: locale === "en" ? l.requirements_en : l.requirements_tr,
        }))
      : defaultPositions;

  /* Build perks from DB content or use defaults */
  const perks = defaultPerks.map((dp, i) => {
    const perkKey = `career_perk_${i + 1}`;
    const section = content?.[perkKey];
    if (section) {
      const title = getLocalizedFieldSync(section, "title", locale);
      const description = getLocalizedFieldSync(section, "subtitle", locale);
      return {
        icon: perkIcons[i] || dp.icon,
        title: title || dp.title,
        description: description || dp.description,
      };
    }
    return dp;
  });

  const [openPosition, setOpenPosition] = useState<string | null>(positions[0]?.id ?? null);

  const applyEmail = settings?.company_email || "bilgi@kismetplastik.com";
        ...
        <div className="relative mx-auto max-w-7xl px-4 lg:px-6">
          <AnimateOnScroll animation="fade-up">
            ...
            <h1 className="mb-4 text-3xl font-extrabold text-white sm:text-4xl lg:text-5xl">
              {t("career_hero", "title", c.heroTitle)}
            </h1>
            <p className="max-w-2xl text-lg text-white/70">
              {t("career_hero", "subtitle", c.heroSubtitle)}
            </p>
          </AnimateOnScroll>
        </div>
      </div>
