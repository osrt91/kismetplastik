...
Bayi profilleri, teklif talepleri, siparişler, iletişim mesajları ve katalog indirme takibi.  
**Kullanım:** Önce `supabase-schema.md` (veya `supabase-schema.sql`) çalıştırılmış olmalı. Bu SQL'i Supabase Dashboard → SQL Editor'a yapıştırıp çalıştırın.
